    ...
    public function getProducts($current_sort = ['sort_priority', 'asc'], $take = 20, $page = 1){
	    if(empty($this->products)){
	    	if($this->enable_redis){
			    $this->products = $this->getRedisProducts($current_sort, $take)->appends(request()->query());
		    }else{
			    $this->products = $this->getDatabaseProducts($current_sort, $take)->appends(request()->query());
		    }
	    }
...
//        $total = $this->products->total();
        ...
        if(!empty($this->product_attributes)){
	        foreach($this->product_attributes as $key => $attribute){
	            ...
	            // Остальные активные фильтры (без текущего атрибута) — общие
	            // для всех его значений, поэтому считаем счётчики одним
	            // сгруппированным запросом на атрибут, а не по одному на значение
	            // (иначе на атрибуте с полусотней значений — полсотни запросов).
	            $base_filter = $filter;
	            unset($base_filter[$attribute->id]);
	            $counts = $this->getAttributeValueCounts($attribute->id, $base_filter);

	            foreach($attribute->values as $i => $attribute_value){
	                $values[$attribute_value->id] = [
	                    'name' => $attribute_value->name,
	                    'value' => $attribute_value->value,
	                    'checked' => isset($filter[$attribute->id]) && in_array($attribute_value->id, $filter[$attribute->id]),
	                    'count' => $counts[$attribute_value->id] ?? 0,
                        'image' => in_array($attribute->type, ['multiple_color_checkboxes', 'single_color_radio']) ? $attribute_value->image : null
	                ];
	                if($with_url)
                        $values[$attribute_value->id]['url'] = $this->getFilterUrl($attribute_value);
	            }

	            // Реально выбираемые варианты — с непустым названием и либо
	            // ненулевым счётчиком, либо уже выбранные. Атрибут с 0 или 1
	            // таким вариантом в сайдбар не имеет смысла выводить: пустой
	            // аккордеон или единственный вариант, снятие/выбор которого
	            // ничего не меняет в выдаче.
	            $usable_count = 0;
	            foreach($values as $value){
	            ...
	            if(!empty($attribute_name) && $usable_count >= 2){
	                $attrs[$attribute->id] = [
	                    'name' => $attribute_name,
                        'unit' => $attribute->unit,
	                    'slug' => $attribute->slug,
...
//					    }
    ...
    /**
     * Количество товаров по каждому значению одного атрибута — одним
     * сгруппированным запросом вместо запроса на каждое значение. Считает
     * количество товаров, которые попадут в выборку, если ЭТО значение
     * атрибута добавить к уже активным $filter (остальные атрибуты не
     * трогаются) — та же семантика, что была у поштучных вызовов
     * get_products_count(), но без N+1.
     *
     * @param int $attribute_id
     * @param array $filter
     * @return array value_id => count
     */
    private function getAttributeValueCounts($attribute_id, $filter){
        $categories = $this->getCurrentCategoriesWithChildrenIds();
        $hash = 'attr_value_counts_'.md5(serialize($categories).serialize($filter).'_'.$attribute_id);

        return Cache::remember($hash, 1440, function () use ($categories, $filter, $attribute_id){
            $query = \Illuminate\Support\Facades\DB::table('products')
                ->join('product_attributes AS attrX', 'products.id', '=', 'attrX.product_id')
                ->where('attrX.attribute_id', $attribute_id)
                ->where('products.visible', 1);

            if(!empty($categories)){
                $query->join('product_categories AS cat', 'products.id', '=', 'cat.product_id');
                $query->whereIn('cat.category_id', $categories);
            }
            ...
            if(!empty($filter)){
                foreach($filter as $attr_id => $values){
                    ...
                    $query->where($alias.'.attribute_id', $attr_id);
                    $query->whereIn($alias.'.attribute_value_id', $values);
                }
            }

            return $query->select('attrX.attribute_value_id', \Illuminate\Support\Facades\DB::raw('COUNT(DISTINCT products.id) as cnt'))
                ->groupBy('attrX.attribute_value_id')
                ->pluck('cnt', 'attrX.attribute_value_id')
                ->toArray();
        });
    }
