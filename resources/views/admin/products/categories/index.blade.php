...
{{-- page title --}}
@section('title', trans('locale.Product categories'))
{{-- vendor style --}}
...
@endsection
@section('content')
    <h1 class="pages-title">{{ trans('locale.Product categories') }}</h1>
    <section class="categories-list-wrapper">
        @if($me->hasAccess(['categories.create']))
            <div class="category-create-btn mb-1">
                <button id="js_add_category" class="btn btn-primary glow invoice-create" type="button" role="button" aria-pressed="true">
                    {{ trans('locale.New add') }}
                </button>
            </div>
        @endif
