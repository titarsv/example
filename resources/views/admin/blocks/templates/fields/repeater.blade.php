        ...
        <div class="accordion fields" data-parent="{{ isset($parent) ? $parent : '' }}" id="basic-list-group{{ $parent_key }}">
            @if(isset($field->fields))
                @foreach($field->fields as $key => $field)
                    @include('admin.blocks.templates.field', ['index' => $key + 1, 'key' => '_secondary'.str_replace(['fields', '[', ']'], ['_', '', ''], $parent).'_'.($key + 1), 'parent_key' => !empty($parent) ? str_replace(['fields', '[', ']'], ['_', '', ''], $parent) : '', 'parent' => $parent."[fields][".($key + 1)."]", 'parent_id' => 'basic-list-group'.$parent_key])
                @endforeach
            @endif
        </div>
    </div>
