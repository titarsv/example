...
{{-- page styles --}}
@section('page-styles')
    <link rel="stylesheet" type="text/css" href="{{asset('css/admin/blocks-templates.css')}}">
@endsection

@section('content')
    <h1 class="pages-title">{{ trans('locale.Block Template') }} "{{ $template->name }}.blade.php"</h1>
    <div class="hidden">
        @include('admin.blocks.templates.field', ['index' => 0, 'key' => 0, 'field' => null, 'parent_key' => '_0', 'parent' => '', 'parent_id' => 'basic-list-group'])
        @include('admin.blocks.templates.fields.select', ['field' => null, 'parent_key' => '_0', 'parent' => '', 'parent_id' => 'basic-list-group'])
        @include('admin.blocks.templates.fields.number', ['field' => null, 'parent_key' => '_0', 'parent' => '', 'parent_id' => 'basic-list-group'])
        @include('admin.blocks.templates.fields.repeater', ['field' => null, 'parent_key' => '_0', 'parent' => '', 'parent_id' => 'basic-list-group'])
        @include('admin.blocks.templates.fields.group', ['field' => null, 'parent_key' => '_0', 'parent' => '', 'parent_id' => 'basic-list-group'])
    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active fade show" id="fields" aria-labelledby="fields-tab" role="tabpanel">
                            <form action="/admin/blocks/template/fields/{{ $template->name }}" method="post" class="js_ajax_form" id="js_blocks_template_form" novalidate>
                                {!! csrf_field() !!}
                                <div class="row mb-2">
                                    <div class="col-md-4">
                                        <fieldset>
                                ...
                                <div id="accordion-icon-wrapper" class="collapse-icon accordion-icon-rotate">
                                    <div class="accordion fields" data-parent="" id="basic-list-group">
                                        @foreach($template->fields as $key => $field)
                                            @include('admin.blocks.templates.field', ['index' => $key + 1, 'key' => $key + 1, 'parent_key' => '', 'parent' => "fields[".($key + 1)."]", 'parent_id' => 'basic-list-group'])
                                        @endforeach
                                    </div>
                                </div>
                                        ...
                                        <div class="email-app-list-wrapper">
                                            <div class="email-app-list">
                                                ...
                                                <div class="email-user-list list-group">
                                                    <form action="/admin/blocks/template/file/{{ $template->name }}" method="post" class="js_ajax_form" novalidate>
                                                        {!! csrf_field() !!}
                        ...
                        <div class="tab-pane fade show" id="preview" aria-labelledby="preview-tab" role="tabpanel">
                            @if(!empty($preview_block))
                                <iframe src="/admin/blocks/template/preview/{{ $template->name }}" style="width:100%;height:75vh;border:1px solid #3b4253;border-radius:4px;"></iframe>
                            @else
                                    ...
                                    <tbody>
                                        @foreach($revisions as $revision)
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                    @if($me->hasAccess(['blocks.write']))
                                                        <button type="button" class="btn btn-sm btn-outline-primary js_restore_revision" data-id="{{ $revision->id }}" data-name="{{ $template->name }}" data-app="blocks">
...
@section('page-scripts')
    ...
    <script src="{{asset('js/admin/blocks-templates.js')}}"></script>
@endsection
