...
@section('title', trans('locale.SEO Content Generation'))

@section('content')
    <h1 class="pages-title">{{ trans('locale.SEO Content Generation') }}</h1>
                ...
                <div class="row mb-2">
                    <div class="col-12">
                        <div class="alert alert-info" role="alert">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <strong>{{ trans('locale.Total SEO Records') }}:</strong> {{ $stats['total_seo_records'] }}
                                </div>
                                <div class="col-md-3">
                                    <strong>{{ trans('locale.Processed SEO Records') }}:</strong> {{ $stats['processed_seo_records'] }}
                                </div>
                                <div class="col-md-3">
                                    <strong>{{ trans('locale.Pending Records') }}:</strong> {{ $stats['pending_records'] }}
                                </div>
                                <div class="col-md-3">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="form-group">
                            <label for="limit">{{ trans('locale.Pages to Process') }}:</label>
                            <div class="input-group">
                                <input type="number" 
                                       ...
                                       min="1" 
                                       max="500"
                                       placeholder="{{ trans('locale.Enter number of pages') }}">
                                <div class="input-group-append">
                                    <button type="button" 
                                            class="btn btn-primary generate-btn"
                                            id="generateBtn">
                                        <span class="spinner-border spinner-border-sm hidden" role="status" aria-hidden="true"></span>
                                        {{ trans('locale.Generate SEO Content') }}
                                    </button>
                                </div>
                            </div>
                            <small class="form-text text-muted">
                                {{ trans('locale.Maximum 500 pages per batch') }}
                            </small>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    function updateStats() {
        $.get('/admin/seo-content/progress', function(data) {
            if (data.stats) {
                $('.alert-info .col-md-3:nth-child(1) strong').next().text(data.stats.total_seo_records);
                $('.alert-info .col-md-3:nth-child(2) strong').next().text(data.stats.processed_seo_records);
                $('.alert-info .col-md-3:nth-child(3) strong').next().text(data.stats.pending_records);
    ...
    $('#generateBtn').click(function() {
        ...
        $.post('/admin/seo-content/generate', {limit: limit}, function(data) {
