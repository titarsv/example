<form action="" method="post" id="js_genetate_ttn_form" class="genetate_ttn_form">
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <select class="form-control" id="payer" name="payer">
                <option value="Sender"{{ $order->total_price >= 8000 ? ' selected' : '' }}>{{ trans('locale.Sender') }}</option>
                <option value="Recipient"{{ $order->total_price < 8000 ? ' selected' : '' }}>{{ trans('locale.Recipient') }}</option>
            </select>
        </div>
    </div>
    <div class="row">
    ...
    <div class="row">
        <div class="col-xs-12">
            <label for="js_np_service_type" class="control-label">{{ trans('locale.Receiving method') }}</label>
            <select class="form-control" id="js_np_service_type" name="service_type">
                <option value="WarehouseWarehouse"{{ $method == 'newpost' ? ' selected' : '' }}>{{ trans('locale.At the branch') }}</option>
                <option value="WarehouseDoors"{{ $method == 'newpost_courier' ? ' selected' : '' }}>{{ trans('locale.By courier') }}</option>
            </select>
        </div>
    </div>
    @if(empty($city_id))
        ...
        <div class="col-xs-12">
            ...
            <select class="form-control" id="warehouse" name="warehouse">
                ...
                @foreach($warehouses as $warehouse)
                    <option value="{{ $warehouse->warehouse_id }}"{{ $warehouse_id == $warehouse->warehouse_id ? ' selected' : '' }}>{{ $warehouse->address_ru }}</option>
                @endforeach
            </select>
        </div>
    </div>
    @if(!empty($destination['warehouse']))
    <div class="row js_np_warehouse{{ $method == 'newpost_courier' ? ' hidden' : '' }}">
        <div class="col-xs-12">
            ...
            <select class="form-control" id="destination_warehouse" name="destination_warehouse">
                ...
                @foreach($destination['warehouse']['options'] as $option)
                    <option value="{{ $option->warehouse_id }}"{{ $option->id == $destination['warehouse']['selected'] ? ' selected' : '' }}>{{ $option->address_ru }}</option>
                @endforeach
            </select>
        </div>
    </div>
    @endif
    <div class="row js_np_courier{{ $method == 'newpost' ? ' hidden' : '' }}">
        <div class="col-xs-6">
            <label for="street" class="control-label">{{ trans('locale.Street') }}</label>
            <input class="form-control" id="street" name="street" value="{{ !empty($delivery['street']) ? $delivery['street'] : '' }}" type="text">
        </div>
        <div class="col-xs-6">
            <label for="house" class="control-label">{{ trans('locale.House number') }}</label>
            <input class="form-control" id="house" name="house" value="{{ !empty($delivery['house']) ? $delivery['house'] : '' }}" type="text">
        </div>
    </div>
    <div class="row js_np_courier{{ $method == 'newpost' ? ' hidden' : '' }}">
        <div class="col-xs-6">
            <label for="apartment">{{ trans('locale.Apartment') }}</label>
            <input class="form-control" id="apartment" name="apartment" value="{{ !empty($delivery['apartment']) ? $delivery['apartment'] : '' }}" type="text">
        </div>
    </div>
    <div class="row">
        <div class="col-xs-12">
            ...
            <select class="form-control" id="cargo_type" name="cargo_type">
                ...
                <option value="Documents">{{ trans('locale.Documents') }}</option>
