@foreach($items as $item)
    <li>
        <div class="checkbox">
            <input type="checkbox" class="menu-item-checkbox" id="edit-menu-page-{{$key}}-{{ $item->id }}" name="menu-item[-{{ $i }}][menu-item-object-id]" value="{{ $item->id }}"/>
            <label class="menu-item-title" for="edit-menu-page-{{$key}}-{{ $item->id }}">
                {{ $item->name }}
            </label>
        </div>
        <input type="hidden" class="menu-item-db-id" name="menu-item[-{{ $i }}][menu-item-db-id]" value="0"/>
        <input type="hidden" class="menu-item-object" name="menu-item[-{{ $i }}][menu-item-object]" value="page"/>
        <input type="hidden" class="menu-item-parent-id" name="menu-item[-{{ $i }}][menu-item-parent-id]" value="{{ $item->parent_id }}"/>
        <input type="hidden" class="menu-item-type" name="menu-item[-{{ $i }}][menu-item-type]" value="post_type"/>
        <input type="hidden" class="menu-item-title" name="menu-item[-{{ $i }}][menu-item-title]" value="{{ $item->name }}"/>
