...
<textarea rows="{{ !empty($rows) ? $rows : 6 }}" autocomplete="off"{!! !empty($max_length) ? ' data-length="'.$max_length.'"' : '' !!}
  ...
  @endif
  {!! !empty($required) ? ' data-validation-required-message="'.trans('locale.Fill this field').'" required' : '' !!}>{{ !empty($value) ? $value : (old($key) ? old($key) : (isset($locale) ? (isset($item) ? $item->localize($locale, $key) : '') : (isset($item) && isset($item->$key) ? $item->$key : ''))) }}</textarea>
@if(!empty($max_length))
