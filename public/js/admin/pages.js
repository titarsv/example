...
$(document).ready(function () {
  ...
  // init data table
  if ($(".pages-data-table").length) {
    let displayLength = window.getCookie('pages_display_length');

    let dataListView = $(".pages-data-table").DataTable({
      ajax: {
            ...
            for(let i in data){
              ...
              if(typeof data[i].type !== 'undefined'){
                if(data[i].type === 'edit'){
                  ...
                  html += '<span class="category-action-edit cursor-pointer js_delete_item" data-endpoint="pages" data-id="'+data[i].id+'" data-name="'+data[i].name+'">'+ text +'</span>';
                }
    ...
    $('#js_pages_list_wrapper').on('change', '[name="DataTables_Table_0_length"]', function () {
      window.setCookie('pages_display_length', $(this).val(), 90);
    });
  ...
  if ($(".order-item-repeater").length) {
    ...
    $('#js_add_page').on('click', function () {
        Swal.fire({
            title: window.localization.js_messages['Enter the page title'],
            input: 'text',
            ...
            confirmButtonText: window.localization.js_messages['Create'],
            cancelButtonText: window.localization.js_messages['Cancel'],
            showLoaderOnConfirm: true,
            cancelButtonClass: "btn btn-danger ml-1",
            preConfirm: function (name) {
                return fetch("/admin/pages/create", {
                    method: 'POST',
                    headers: {
    ...
    $(document).on('click', '.remove-item', function(){
        ...
        if(prev.length){
            ...
            repeater.find('.js_picture_wrapper').each(function(){
                $(this).find('.js_upload_image_button').show();
                $(this).children('div').not('.js_upload_image_button').remove();
            });
