$(document).ready(function () {
    ...
    // init data table
    if ($(".pages-templates-data-table").length) {
        let displayLength = window.getCookie('pages_templates_display_length');

        let dataListView = $(".pages-templates-data-table").DataTable({
            ajax: {
                url: '/admin/pages/templates/list',
                type: "POST"
            },
                    ...
                    render: function ( data, type, row ) {
                        if(row.missing){
                            return '<i class="bx bx-error-circle text-danger mr-1" data-toggle="tooltip" data-placement="bottom" title="'
                                + __('Template file not found, referenced by :count page(s)', {count: row.entries_count})
                                + '"></i>' + data;
                        }
                        return data;
                    }
        ...
        $('#js_pages_templates_list_wrapper').on('change', '[name="DataTables_Table_0_length"]', function () {
            window.setCookie('pages_templates_display_length', $(this).val(), 90);
        });

        dataListView.on( 'draw.dt', function(){
            $('.pages_templates-data-table [data-toggle="tooltip"]').tooltip();
        });
    }
    ...
    // Дублирование шаблона ("Save as new") из списка шаблонов страниц
