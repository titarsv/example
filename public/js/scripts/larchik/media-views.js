...
media.view.FieldsetPriorityList = __webpack_require__( 101 );
...
Toolbar = View.extend(/** @lends wp.media.view.Toolbar.prototype */{
	...
	initialize: function() {
		...
		this.primary   = new wp.media.view.FieldsetPriorityList();
		this.secondary = new wp.media.view.PriorityList();
		this.header_menu = new wp.media.view.PriorityList();
        this.header_menu.set('primary', this.primary, {});
		this.primary.$el.addClass('form-group position-relative has-icon-left m-0').append('<div class="form-control-position"><i class="bx bx-search"></i></div>');
		this.secondary.$el.addClass('app-file-header-icons');
        this.header_menu.$el.addClass('app-file-header-search flex-grow-1').prepend('<div class="sidebar-toggle d-block d-lg-none"><i class="bx bx-menu"></i></div>');

		this.views.set([ this.header_menu, this.secondary ]);
...
var Button = wp.media.View.extend(/** @lends wp.media.view.Button.prototype */{
	...
	render: function() {
		...
		var classes = [ 'btn', this.className ],
			model = this.model.toJSON();

		if ( model.style ) {
			classes.push( 'btn-' + model.style );
		}

		if ( model.size ) {
			classes.push( 'btn-' + model.size );
		}
...
Menu = PriorityList.extend(/** @lends wp.media.view.Menu.prototype */{
	tagName:   'div',
	className: 'app-file-sidebar sidebar-content d-flex',
	property:  'state',
	ItemView:  MenuItem,
	region:    'menu',
	template:  wp.template('sidebar'),
	...
	*/
	...
	prepare: function() {
		...
		data.filters.all = {
			...
			icon: 'morph-folder.svg',
			color: 'primary',
			count: wp.media.view.settings.attachmentCounts.all,
			props: {
				status:  null,
				type:    null,
				uploadedTo: null,
		...
		// data.filters.important = {
		// 	text:  l10n.important,
		// 	icon: 'morph-star.svg',
		// 	color: 'secondary',
		// 	count: wp.media.view.settings.attachmentCounts.important,
		// 	props: {
		// 		uploadedTo: null,
		// 		status:     null,
		// 		type:       null,
		...
		if ( wp.media.view.settings.mediaTrash &&
			...
			data.filters.trash = {
				...
				icon: 'trash.svg',
				color: 'danger',
				count: wp.media.view.settings.attachmentCounts.trash,
				props: {
					uploadedTo: null,
					status:     'trash',
					type:       null,
		...
		_.each( wp.media.view.settings.mimeTypes || {}, function( text, key ) {
			data.mimeTypes[ key ] = {
				text: text,
				icon: typeof wp.media.view.settings.mimeIcons[key] !== 'undefined' ? wp.media.view.settings.mimeIcons[key] : null,
				props: {
					status:  null,
					type:    key,
					uploadedTo: null,
	...
	ready: function() {
		...
		this.$el.find(".livicon-evo").updateLiviconEvo({
			strokeColor: "#727E8C"
		});

		var app_file_sidebar_left = $(".app-file-sidebar-left");

		if (app_file_sidebar_left.length > 0) {
			var app_file_sidebar_content = new PerfectScrollbar(".app-file-sidebar-content");
		}

		app_file_sidebar_left.find('.list-group-item-action').click(function(){
			$('#media-attachment-filters').val($(this).data('index')).change();
		});
	},
...
Attachments = View.extend(/** @lends wp.media.view.Attachments.prototype */{
	tagName:   'ul',
	className: 'app-file-content p-2 ps',
	...
	ready: function() {
		...
		new PerfectScrollbar(".app-file-content");
		this.scroll();
	},
...
AttachmentFilters = wp.media.View.extend(/** @lends wp.media.view.AttachmentFilters.prototype */{
	tagName:   'select',
	className: 'form-control ml-sm-50',
...
AttachmentsBrowser = View.extend(/** @lends wp.media.view.AttachmentsBrowser.prototype */{
	tagName:   'div',
	className: 'attachments-browser app-file-area',
	...
	createToolbar: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			toolbarOptions.className = 'app-file-header';
		}
		...
		*/
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			...
			this.toolbar.set( 'deleteSelectedButton', new wp.media.view.DeleteSelectedButton({
                ...
                text: '<i class="livicon-evo cursor-pointer livicon-evo-holder" data-options="name: trash.svg; size: 24px; style: lines; strokeColor:#596778; duration:0.85;" style="visibility: visible; width: 24px;"></i>',
				controller: this.controller,
				priority: -60,
				click: function() {
					var changed = [], removed = [],
	...
	show: function() {
		if ( ! this.spinnerTimeout ) {
			this.spinnerTimeout = _.delay(function( $el ) {
				$el.html('<span class="spinner-border"></span>').addClass( 'is-active' );
			}, this.delay, this.$el );
		}
...
/**
 * wp.media.view.FieldsetPriorityList
 *
 * @memberOf wp.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
var FieldsetPriorityList = wp.media.view.PriorityList.extend(/** @lends wp.media.view.FieldsetPriorityList.prototype */{
	tagName:   'fieldset'
});

module.exports = FieldsetPriorityList;
