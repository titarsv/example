	...
	sync: function( method, model, options ) {
		...
		// This will permanently delete an attachment.
			...
			return wp.media.ajax( options ).done( function() {
				this.destroyed = true;
                var trash = $('[data-group="filters"][data-index="trash"]');
                var trash_badge = trash.find('span');
                var count = trash_badge.length ? parseInt(trash_badge.text()) : 0;
                if(this.attributes.status == 'trashed'){
                    count--;
                }else{
                    count++;
                }
                if(trash_badge.length){
                    if(count){
                        trash_badge.text(count);
                    }else{
                        trash_badge.remove();
                    }
                }else{
                    trash.append('<span class="badge badge-light-danger badge-pill badge-round float-right mt-50">'+count+'</span>');
                }
			}).fail( function() {
