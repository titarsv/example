...
var Router = Backbone.Router.extend(/** @lends wp.media.view.MediaFrame.Manage.Router.prototype */{
	routes: {
		'/media?item=:slug&mode=edit': 'editItem',
		'/media?item=:slug':           'showItem',
		'/media?search=:query':        'search',
		'/media':                      'reset'
	},
...
SelectModeToggle = Button.extend(/** @lends wp.media.view.SelectModeToggle.prototype */{
    ...
    ready: function() {
        if(!this.$el.hasClass('select-mode-toggle-button')){
            this.$el.addClass( 'select-mode-toggle-button ml-sm-50' ).html('<i class="livicon-evo cursor-pointer livicon-evo-holder" data-options="name: check-alt.svg; size: 24px; style: lines; strokeColor:#5A8DEE; eventOn:grandparent; duration:0.85;"></i>');
    ...
    toggleBulkEditHandler: function() {
        ...
        if ( this.controller.isModeActive( 'select' ) ) {
			...
			toolbar.$('.livicon-evo').updateLiviconEvo({
				strokeColor: "#727E8C"
			});
        } else {
            // this.model.set( {
...
DeleteSelected = Button.extend(/** @lends wp.media.view.DeleteSelectedButton.prototype */{
	...
	filterChange: function( model ) {
		if ( 'trash' === model.get( 'status' ) ) {
            ...
            this.model.set( 'text', '<i class="livicon-evo cursor-pointer livicon-evo-holder" data-options="name: rotate-left.svg; size: 24px; style: lines; strokeColor:#596778; duration:0.85;" style="visibility: visible; width: 24px;"></i>' );
            this.$el.find(".livicon-evo").updateLiviconEvo({
                strokeColor: "#727E8C"
            });
		} else if ( wp.media.view.settings.mediaTrash ) {
			// this.model.set( 'text', l10n.trashSelected );
			this.model.set( 'text', '<i class="livicon-evo cursor-pointer livicon-evo-holder" data-options="name: trash.svg; size: 24px; style: lines; strokeColor:#596778; duration:0.85;" style="visibility: visible; width: 24px;"></i>' );
            this.$el.find(".livicon-evo").updateLiviconEvo({
                strokeColor: "#727E8C"
            });
		} else {
    ...
    ready: function() {
        if(!this.$el.find('.livicon-evo').length){
            this.$el.html('<i class="livicon-evo cursor-pointer livicon-evo-holder" data-options="name: trash.svg; size: 24px; style: lines; strokeColor:#596778; duration:0.85;" style="visibility: visible; width: 24px;"></i>');
            this.$el.find(".livicon-evo").updateLiviconEvo({
                strokeColor: "#727E8C"
            });
        }
