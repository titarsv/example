	...
	api = wpNavMenu = {
			...
			$.fn.extend({
				...
				addSelectedToMenu : function(processMethod) {
					...
					return this.each(function() {
						...
						t.find( '.button-controls .spinner-border' ).addClass( 'is-active' );
						...
						api.addItemToMenu(menuItems, processMethod, function(){
							...
							t.find( '.button-controls .spinner-border' ).removeClass( 'is-active' );
						});
					});
				},
		...
		attachThemeLocationsListeners : function() {
			...
			loc.find('input[type="submit"]').click(function() {
				...
				loc.find( '.spinner-border' ).addClass( 'is-active' );
				$.post( ajaxurl, params, function() {
					loc.find( '.spinner-border' ).removeClass( 'is-active' );
				});
				return false;
			});
		},
		...
		updateQuickSearchResults : function(input) {
			...
			$( '.spinner-border', panel ).addClass( 'is-active' );
		...
		addCustomLink : function( processMethod ) {
			...
			// Show the ajax spinner
			$( '.customlinkdiv .spinner-border' ).addClass( 'is-active' );
			this.addLinkToMenu( url, label, processMethod, function() {
				// Remove the ajax spinner
				$( '.customlinkdiv .spinner-border' ).removeClass( 'is-active' );
		...
		addGroup : function( processMethod ) {
			var label = $('#group-menu-item-name').val();

			processMethod = processMethod || api.addMenuItemToBottom;

			// Show the ajax spinner
			$( '.groupdiv .spinner-border' ).addClass( 'is-active' );
			this.addGroupToMenu( '', label, processMethod, function() {
				// Remove the ajax spinner
				$( '.groupdiv .spinner-border' ).removeClass( 'is-active' );
				// Set group form back to defaults
				$('#group-menu-item-name').val('').blur();
			});
		},
		...
		addGroupToMenu : function(url, label, processMethod, callback) {
			processMethod = processMethod || api.addMenuItemToBottom;
			callback = callback || function(){};

			api.addItemToMenu({
				'-1': {
					'menu-item-type': 'group',
					'menu-item-title': label
				}
			}, processMethod, callback);
		},
		...
		attachTabsPanelListeners : function() {
			$('#menu-settings-column').bind('click', function(e) {
				...
				if ( target.hasClass('nav-tab-link') ) {
					...
					$('.current', wrapper).removeClass('current');
					...
					if ( selectAreaMatch && selectAreaMatch[1] ) {
						items = $('#' + selectAreaMatch[1] + ' .tabs-panel-active .checkbox input');
                    ...
                    }else if ( e.target.id && 'submit-groupdiv' == e.target.id ){
                        api.addGroup( api.addMenuItemToBottom );
		...
		processQuickSearchQueryResponse : function(resp, req, panel) {
			...
			if( ! $items.length ) {
				...
				$( '.spinner-border', panel ).removeClass( 'is-active' );
				wrapper.addClass( 'has-no-menu-item' );
				return;
			}
