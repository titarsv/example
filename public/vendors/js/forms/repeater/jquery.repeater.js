...
// Brian Detering <BDeterin@gmail.com> (http://www.briandetering.net/)
    ...
    $.fn.repeater = function (fig) {
        ...
        $(this).each(function () {
            ...
            var clone = fig.clone || function (cloneElement) {
                cloneElement();
            };
            ...
            var cloneItem = function ($this, data) {
                let originalFig = fig;
                fig = fig.parentFig;
                $list = $this.parents('[data-repeater-list]').first();
                var d = {};
                var parent = $this.parents('[data-repeater-item]').eq(0);
                parent.find('input, select, textarea').each(function(index, obj){
                    d[index] = $(this).val();
                });
                ...
                $item.find('input, select, textarea').each(function(index, obj){
                    if(obj.nodeName === 'SELECT'){
                        $(this).find('option[value="'+d[index]+'"]').prop('selected', true);
                    }else{
                        $(this).val(d[index]);
                    }
                });
                $filterNested($item.find('[data-repeater-clone]'), fig.repeaters).off('click').click(function () {
                    cloneItem($(this));
                });
                $list = $self.find('[data-repeater-list]').first();
                show.call($item.get(0));
                clone.call($item.get(0), function(){});
                fig = originalFig;
            };
            ...
            $filterNested($self.find('[data-repeater-clone]'), fig.repeaters).off('click').click(function () {
                cloneItem($(this));
            });
